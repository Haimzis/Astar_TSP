...
def hashed_best_first_graph_search(problem, f, best_known_solution_value, display=False, ):
    ...
    msg = ''
    node = Node(problem.initial)
    node.depth = len(problem.initial) - 1
    node.path_cost = problem.value(problem.initial)
    ...
    while frontier:
        node = frontier.pop()
        frontier_count -= 1
        print(node.state, ' depth: ', node.depth)
        if problem.goal_test(node.state):
            if display:
                msg = "{0} paths have been expanded and {1} paths remain in the frontier"\
                    .format(explored_count, frontier_count)
            return node, msg
        explored_count += 1
        for child in node.expand(problem):
            frontier_count += 1
            if f(child) < best_known_solution_value:
                frontier.append(child)
    return None
...
def astar_search(problem, best_known_solution_value, h=None, display=False):
    """A* search is best-first graph search with f(n) = g(n)+h(n).
    You need to specify the h function when you call astar_search, or
    else in your Problem subclass."""
    h = memoize(h or problem.h, 'h')
    return hashed_best_first_graph_search(problem, lambda n: n.path_cost + h(n), best_known_solution_value, display)
